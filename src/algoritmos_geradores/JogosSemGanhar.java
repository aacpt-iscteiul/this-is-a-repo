	...
	public void gerar() {
		...
		while (contador < contagens) {
			...
			if (num < 45 - 2 * no_wins) // sair na posicao 0
				pos[0] += 1;
			else if (num >= 45 - 2 * no_wins && num < 47.5 - no_wins) // sair na posicao 1
				pos[1] += 1;
			else if (num >= 47.5 - no_wins && num < 52.5 + no_wins) // sair na posicao 2
				pos[2] += 1;
			else if (num >= 52.5 + no_wins && num < 55 + 2 * no_wins) // sair na posicao 3
				pos[3] += 1;
...
//				Thread.sleep(1);
	...
	private void escreveFicheiro() {
		try {
			PrintWriter outFile = new PrintWriter(new FileWriter("jogossemganhar" + nowins + "_" + contagens + ".txt"));
		...
		System.out.println("jogossemganhar" + nowins + "_" + contagens + ".txt escrito em disco");
	}

	public static void main(String[] args) {
		int contagens = 10000;
		final int JOGOS_SEM_GANHAR = 5;
